...
int initVM(InitArgs *args) {
    ...
    status = initialiseHooks(args) &&   // 这个钩子的初始化似乎是jni的
             initialiseProperties(args) &&  // 初始化命令行参数的赋值
             initialiseAlloc(args) &&   // 感觉像是初始化堆栈和线程
             initialiseThreadStage1(args) &&    // 线程的初始化
             initialiseUtf8() &&    // 其实是初始化了hashTable
             initialiseSymbol() &&  // 初始化标志
             initialiseClassStage1(args) &&     // 初始化类加载
             initialiseDll(args) &&     // 设置dll的path
             initialiseMonitor() &&     // 初始化了TODO
             initialiseString() &&      // 初始化String这个类
             initialiseException() &&    // 初始化异常类
             initialiseNatives() &&     //
             initialiseAccess() &&      //
             initialiseFrame() &&       //
             initialiseJNI() &&     //
