...
typedef struct object Class; // 代表类的类型
...
struct methodblock {
   ...
   u2 max_locals;       //
   u2 args_count;       // 参数数量
   u2 throw_table_size;     // 抛出异常数量
   u2 *throw_table;     // 异常数量
   void *code;          // java虚拟机字节码
   int code_size;       // 虚拟机字节码的长度
   union {
       struct {
...
typedef struct classblock {
   ...
   u1 state;                //
   u2 flags;                //
   u2 access_flags;         // 访问标志,access_flags 是一种掩码标志,用于表示某个类或者接口的访问权限 即public。。。。
   u2 declaring_class;      //
   u2 enclosing_class;      // enclosing的这两项是当且仅当当前类为局部类或者匿名类的时候才有效
   u2 enclosing_method;
   u2 inner_access_flags;   //
   u2 fields_count;         // 类变量数量
   u2 methods_count;        // 方法数量
   u2 interfaces_count;     // 实现的接口数量
   u2 inner_class_count;    // 我觉得是内部类数量？？？？
   u2 constant_pool_count;  // 常量池的长度？？？
   int object_size;         // 类长度是个什么鬼
   ...
   char *name;                  // 类名
   char *signature;             // 类签名，很重要的一个东西
   char *source_file_name;      // 源文件名
   Class *super;                // 父类
   FieldBlock *fields;          // 变量集合
   MethodBlock *methods;        // 方法集合
   Class **interfaces;          // 接口集合
   MethodBlock **method_table;  //
   ITableEntry *imethod_table;  //
   Object *class_loader;        // 所用的classLoader
   u2 *inner_classes;           // 内部类
   char *bootstrap_methods;     // 指向 CONSTANT_MethodHandle 结构的索引值
   ExtraAttributes *extra_attributes;   //
   union {
       Class *element_class;
       RefsOffsetsEntry *refs_offsets_table;
   };
   ConstantPool constant_pool;  // 常量池，似乎这玩意比较重要哈
...
typedef struct InitArgs {
    ...
    Property *commandline_props;    // 命令行
    int props_count; // 命令数量
