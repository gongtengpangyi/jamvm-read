...
#define COMPARE(ptr1, ptr2, hash1, hash2) (hash1 == hash2) && \
    ...
    // 这里我觉得是根据classLoader是系统ClassLoader还是子ClassLoader来区分放在哪个hashTable中
...
/**
 * 这个？准备class？？？ 感觉似乎是给java.lang.class做处理的
 * @param class
 */
...
/**
 * 设置属性数据
 * 这里是一系列内存操作。。。可以说是比较麻烦的
 *
 * 我觉得差不多大概的意思是读取数据写到常量池里面吧！
 */
...
/**
 * 查找类的方法
 * @param classname 类名称（以正斜杠为隔开的）
 * @return
 */
Class *findSystemClass0(char *classname) {

    // 从已经加载了的hash表里获取class
    ...
    for (j = 0, pntr = cp; i > 0; i--) {
        ...
        if (stat(start, &info) == 0) {
            if (S_ISDIR(info.st_mode)) {
                ...
                if (len > max)
                    max = len;
            } else if ((bootclasspath[j].zip = processArchive(start)) == NULL)
                continue;
